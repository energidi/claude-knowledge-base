...
| `Ancestor_Id_Tail_Index__c` | Long Text 32768 | Internal engine field. Pipe-delimited 6-char tails (`.right(6)`, auto-number suffix) of ancestor `Metadata_Id__c` values. Used as a fast pre-screen before the expensive full `Ancestor_Path__c` string search. A match triggers the conclusive `Ancestor_Path__c` confirmation; no match skips it. Do not edit manually. |
...
- `Ancestor_Id_Tail_Index__c` 6-char tail has negligible false-positive probability. `Ancestor_Path__c` always used to confirm before setting `Is_Circular__c = true`.
