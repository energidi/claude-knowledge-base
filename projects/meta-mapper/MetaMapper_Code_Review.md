...
**Phase:** 4 - Engine Core  
**Last Updated:** July 20, 2026 (Round 84 - sf-orchestrator full pass, all 8 lenses: 18 of 19 findings applied - 1 Critical, 5 High, 9 Medium, 3 Low; 1 Low finding (kebab-case LWC event renames) found technically infeasible under the LWC framework and reverted)
**Date:** May 23, 2026
...
| Naming (LWC custom events)                | 15 custom events across 8 LWC components use run-together lowercase (`tabready`, `nodeselected`, `showtoast`, etc.) instead of the naming standard's kebab-case (`tab-ready`, `node-selected`, `show-toast`). Renaming was attempted and reverted.                                                                                                                                                                                                                          | Round 84         | **Technically infeasible, not a style choice deferred.** The LWC template compiler rejects hyphenated event types in declarative `on{event}` bindings: attempting `onshow-toast={handler}` in `metaMapperApp.html` produced `LWC1056: Invalid event type "onshow-toast". Event type must begin with a lower-case alphabetic character and contain only lower-case alphabetic characters, underscores, and numeric characters.` Every parent-child listener in this app is wired declaratively, so a kebab-case rename would break all of them. Do not re-attempt this rename; the naming standard's kebab-case rule does not apply to LWC custom event types constrained by the template compiler. |
...
| ID               | Finding                                                                                | Reason for rejection                                                                                                                                                       |
| ---------------- | -------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Grok Critical #1 | "Supplemental DML skip: un-mark allSupplementalNodes parents"                          | Re-submission of R28 analysis. The un-mark target (`toUpsert` items' Parent_Dependency\_\_c) is the wrong set of records. Documented in R28 rejection.                     |
...
| #   | Class                            | Severity | Fix                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| --- | -------------------------------- | -------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| 1   | _(new)_ `DependencyCleanupBatch` | Critical | Class was missing entirely. Failed and Cancelled job cleanup never ran because `DependencyCleanupScheduler` had no class to instantiate. Created with `Database.Stateful`, `MAX_BATCH_SUBMISSIONS = 4` guard, no DML in `execute()`, CMDT-sourced chunk size.                                                                                                                                                                                                                                                            |
...
| 4   | `DependencyQueueable`            | Medium   | Supplemental cross-execution dedup query `nodeSelector.dedupForResults()` had no SOQL budget guard. The call could push query count past the `HANDLER_SOQL_RESERVE` ceiling, leaving insufficient budget for subsequent work. Added `(Limits.getLimitQueries() - Limits.getQueries()) < HANDLER_SOQL_RESERVE` guard; skips dedup with a warning notice when budget is low (Component_Uniqueness_Key\_\_c external ID prevents duplicate creation).                                                                       |
...
DROPPED: #12 (Stall_Pause_Threshold**c rename - Round 46 already renamed Empty_Cycle_Pause_Threshold**c → Stall_Pause_Threshold\_\_c deliberately).
...
- 7.  Computes Results\_\_c (versioned JSON map; trims lowest-count keys if needed)
- 8.  Updates job: Result_File_Id**c, Results**c, Status=Completed, Status_Closed_At\_\_c
...
- job transitions to Completed. Reads Results\_\_c, builds a plain-English
- Scan_Summary_Text\_\_c string, and updates the job record.
-
...
## Round 84 Fixes Applied

Full sf-orchestrator review (all 8 lenses: architecture, UX, naming, security, performance, testing, automation, static analysis). 18 of 19 findings applied (1 Critical, 5 High, 9 Medium, 3 Low; 1 Low finding found technically infeasible - see Known Skipped Findings). Overall verdict before fixes: NO-GO (1 Critical UX finding). Security and Automation lenses returned zero findings (clean passes).

**Critical (UX - JSON export silently drops filtered-out ancestor subtrees):**
- Finding 1 (`metaMapperExport.js:118-140`, `metaMapperResults.html`): the "Download Complete Hierarchy" JSON export walked `filteredNodes` (the on-screen filtered set) instead of the full node list, so applying a type filter that excludes an ancestor's type silently orphaned that ancestor's entire subtree from the export - even though the descendants were still visible on screen. Fixed: `metaMapperResults.html` now passes `nodes={allNodes}` (unfiltered) to `c-meta-mapper-export`; filtering remains scoped to the on-screen Tree/Graph only.

**High (UX/Naming - stale "Report" scan-root option; incomplete Round 83 cascade):**
- Finding 2 (`metaMapperSearch.js` `TYPE_OPTIONS`): Round 83 removed `'Report'` from `DependencyJobController.SUPPORTED_TYPES` server-side but never removed it from the LWC's own type picklist, so users could select it and hit a confusing server rejection. Fixed: removed the `Report` entry from `TYPE_OPTIONS`.
- Finding 7 (`metaMapperSearch.js`/`.html`): Round 83's `activeFlowsOnly` -> `isActiveFlowsOnly` rename only touched the outbound `createJob()` param - the component's own `@track` property, its HTML `checked` binding, and the change-handler assignment still used the old name. Fixed: renamed the property, binding, and handler assignment to `isActiveFlowsOnly`.
- Finding 16 (`MetadataDependencyServiceTest.cls`): stale `activeFlowsOnly` wording in a test method name, an inline comment, and an assertion message from the same incomplete Round 83 cascade. Fixed: renamed `filterInactiveFlows_activeFlowsOnlyFalse_returnsImmediately` -> `filterInactiveFlows_isActiveFlowsOnlyFalse_returnsImmediately` and `filterInactiveFlows_activeFlowsOnly_dropsInactiveRetainsActive` -> `filterInactiveFlows_isActiveFlowsOnly_dropsInactiveRetainsActive`; updated the comment and assertion message.

**High (UX - deep-link to a missing node opens a blank panel with no explanation):**
- Finding 3 (`metaMapperResults.js:loadResults()`): a `?jobId=X&nodeId=Y` deep link where `Y` is absent from the result set opened `metaMapperComponentDetailsPanel` with no data and no explanation - the spec-mandated toast never fired. Fixed: `loadResults()` now checks `nodeMap.has(_pendingNodeId)` before opening the panel; on a miss it dispatches `showtoast` with "The linked component could not be found in this scan result." instead of selecting the node.

**High (UX - Cancel confirmation modal has no Escape-key dismiss):**
- Finding 4 (`metaMapperProgress.html`/`.js`): the "Stop this analysis?" modal was the only modal in the app without an `onkeydown` Escape handler. Fixed: added `onkeydown={handleCancelModalKeyDown}` to the dialog `<section>`, mirroring the existing Learn More/Tour modal pattern - Escape now calls `handleKeepRunning()`.

**High (Testing - DependencyQueueable never tested with a multi-node batch):**
- Finding 5 (`DependencyQueueableTest.cls`): every existing test inserted exactly one unprocessed node before enqueueing, so `nextUnprocessed()` never returned more than one row in a single execution - batch accumulation, per-parent dedup, and the mid-loop guard across multiple parents were unverified. Fixed: added `execute_multipleUnprocessedNodesInBatch_processesAllAndAccumulatesResultsInOneBulkUpsert()`, inserting 5 distinct parent nodes and asserting all 5 are marked fetched and all 5 children are bulk-inserted from one callout.

**High (Testing - package.xml export has zero test coverage):**
- Finding 6 (`metaMapperExport.js` `handleDownloadPackageXml`/`_buildPackageXml`): no test exercised the namespace-exclusion contract or `<types>` grouping for this export path. Fixed: added a `describe('package.xml export', ...)` block covering namespace exclusion, `<types>` grouping, the `package.xml` filename, and build-failure isolation (toast fired, no throw, CSV/JSON unaffected).

**Medium (Architecture - heap pre-check sample is systematically biased toward shallow nodes):**
- Finding 8 (`ScanResultFileQueueable.cls:141-146`): the heap sample read only the first 50 nodes of a list ordered `Dependency_Depth__c ASC`, always the shallowest nodes with the shortest `Ancestor_Path__c`/`Ancestor_Id_Index__c` values - underestimating average serialized size for deep, narrow trees. Fixed: sample with a stride across the full list (`stride = nodes.size() / sampleSize`) instead of the first N, so both shallow and deep nodes are represented in the average.

**Medium (Architecture - CMT entity cap "deferred" wording is inaccurate; no cross-execution cursor exists):**
- Finding 9 (`CustomFieldDependencyHandler.cls`, `ApexClassDependencyHandler.cls`): `MAX_CMT_ENTITIES_PER_EXECUTION = 30` logged entities beyond the cap as "deferred," but no cursor/offset persists across Queueable self-chains and `getCmtEntities()` returns the same deterministic order every call - entities beyond the 30th are permanently excluded for the whole job, not merely delayed. Fixed: corrected the diagnostic wording in both handlers to say "excluded from this job's CMT scan (not merely delayed - no cross-execution cursor is persisted)." A full persisted-cursor mechanism was considered and deferred as a larger architectural change outside a fix-pass diff.

**Medium (Architecture - unresolvable scan target completes silently as a zero-result scan):**
- Finding 10 (`DependencyQueueable.cls` Step 5a): when `resolveRootId()` cannot resolve the scan target (mistyped API name, deleted component, wrong parent object), no diagnostic was logged - the job silently completed as an ordinary zero-result scan, indistinguishable from a legitimate empty scan. Fixed: logs "Target component '[name]' could not be resolved via the Tooling API. Verify the API name and parent object are correct." to `Scan_Diagnostic_Log__c`. Added `execute_rootIdUnresolvable_logsDiagnosticNotice()` to `DependencyQueueableTest.cls`.

**Medium (Naming - CMDT field labels don't match CLAUDE.md's documented Settings UI labels):**
- Finding 11: 5 field labels (`Retention_Hours__c`, `Is_Platform_Events_Disabled__c`, `Stall_Pause_Threshold__c`, `Max_Stored_Jobs__c`, `Min_Free_Storage_MB__c`) were API-name echoes or otherwise diverged from the documented admin-facing labels. Fixed: updated all 5 `<label>` values to match CLAUDE.md's Settings UI table exactly (e.g. "Retention Hours" -> "Keep failed/cancelled jobs for (hours)").

**Medium (Performance - regex fallback in findValidationRuleReferences has no mid-loop CPU check):**
- Finding 12 (`CustomFieldDependencyHandler.cls:414-422`): the per-rule CPU guard fires once per rule, but the inner per-field regex-fallback loop had no CPU check of its own - on orgs with many active ValidationRules and wide CustomField batches this could re-approach the O(rules x fields) cost the inverted-token pass was meant to eliminate. Fixed: added a CPU checkpoint every 10 fields inside the loop; on trip, logs a diagnostic, breaks the field loop, and breaks the outer rule loop.

**Medium (Testing - MetaMapperSettingsProvider has no direct unit test):**
- Finding 13: the shared static per-transaction CMDT cache used by the engine, controller, notification service, and all supplemental handlers had no test verifying cache-identity reuse or the `clearCache()` test-injection seam other test classes rely on. Fixed: added `MetaMapperSettingsProviderTest.cls` with two tests proving same-transaction cache reuse (via an in-memory mutation check) and that `clearCache()` forces a fresh read.

**Medium (Testing - validateFilters "all types invalid" branch untested):**
- Finding 14 (`metaMapperFilters.js:33`): existing coverage only asserted `types === []` on this branch, not the other 6 fields' reset to `DEFAULT_FILTERS`. Fixed: added a test asserting full `toEqual(DEFAULT_FILTERS)` when every stored type is invalid.

**Medium (Static Analysis - Prettier formatting drift across 208 files):**
- Finding 15: `npm run prettier:verify` reported drift across 208 files spanning Apex, LWC, and metadata XML. Fixed: ran `npm run prettier` at the repo root; `prettier:verify` now passes clean.

**Low (UX - confidence threshold filter input has no clamping):**
- Finding 17 (`metaMapperFilterPanel.js:66-69`): typing `150` silently hid every supplemental node with no explanation. Fixed: clamped via `Math.min(100, Math.max(0, val))` in `handleConfidenceChange`.

**Low (Performance - MetadataTypeDescribeService.getCmtEntities() truncation is silent):**
- Finding 19 (`MetadataTypeDescribeService.cls:76-87`): the `LIMIT 2000` cap had no truncation diagnostic, unlike the `MAX_CMT_ENTITIES_PER_EXECUTION` cap which does log. Fixed: added a check for `cachedCmtEntities.size() == 2000` that adds a diagnostic notice to the existing `describeFailed` channel (surfaced to `Scan_Diagnostic_Log__c` via `drainDescribeErrors()`).

**NOT APPLIED - technically infeasible (Naming - kebab-case LWC event renames):**
- Finding 18 (15 events across 8 LWC components): attempted rename to kebab-case (`tabready` -> `tab-ready`, etc.) per the naming standard. Reverted after the LWC template compiler rejected the hyphenated declarative binding `onshow-toast={handleShowToast}` in `metaMapperApp.html` with `LWC1056: Invalid event type`. LWC's `on{event}` template binding syntax only permits lowercase alphanumerics and underscores in the event type - hyphens are not supported for declarative listeners. Since every parent-child event listener in this app is wired declaratively, this rename is not achievable without switching to imperative `addEventListener()` wiring everywhere, which is out of scope for a naming fix. Added to Known Skipped Findings below as a permanent technical constraint, not a deferred style choice.

**Rollback notes (Critical/High findings):**
- Finding 1: revert `metaMapperResults.html` `nodes={allNodes}` back to `nodes={filteredNodes}` on the `c-meta-mapper-export` binding.
- Finding 2: revert `metaMapperSearch.js` `TYPE_OPTIONS` to re-include `{ label: 'Report', value: 'Report' }`.
- Finding 3: revert the `nodeMap.has()` guard and `showtoast` dispatch in `metaMapperResults.js:loadResults()`.
- Finding 4: revert `onkeydown={handleCancelModalKeyDown}` in `metaMapperProgress.html` and remove `handleCancelModalKeyDown()` from `metaMapperProgress.js`.
- Finding 5: revert the new test in `DependencyQueueableTest.cls`.
- Finding 6: revert the new `describe('package.xml export', ...)` block in `metaMapperExport.test.js`.

**CLAUDE.md updates:** documented the corrected CMT-cap diagnostic wording ("excluded," not "deferred") for `CustomFieldDependencyHandler`/`ApexClassDependencyHandler`; documented the Step 5a root-resolution diagnostic notice in `DependencyQueueable`'s description; documented the heap-sample stride change in `ScanResultFileQueueable`'s description; noted that `metaMapperExport` now receives `allNodes` (unfiltered) rather than `filteredNodes`.

**Verification:** `npm run lint` - clean, zero violations. `npm run prettier:verify` - clean. `npx jest --config jest.config.js` (full suite) - 9 suites, 117/117 tests passing. Apex-side changes reviewed against existing test patterns but not executed against the org (requires deployment - gated by explicit user approval per the Salesforce Deploy Gate).
