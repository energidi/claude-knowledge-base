...
### Round 14 Fixes Applied

| # | Class | Change |
|---|---|---|
| D1 | CustomFieldHandler | Removed dead `nodes` parameter from `findCmtRecordReferences` and its call site: the L3 map-refactoring round removed all reads of this parameter but left it in the signature, creating an unused argument on every call |
| N1 | SupplementalResult | Added `maxLength` null guard in `appendErrorsSafe`: `safeThreshold` was guarded (M1) but `maxLength` was not; a null `maxLength` throws `NullPointerException` on the `safeLimit` subtraction |
| L2D | MetaMapperDescribeCache | Added size check after `getCmtEntities` query: when result equals `LIMIT 2000`, emits a diagnostic to `describeFailed` so callers see the truncation in `Error_Status_Message__c` |
| VR-CPU | CustomFieldHandler | Added CPU guard inside the `for (String lowerField : fieldApiNamesLower)` loop in `findValidationRuleReferences` with a `vrCpuExceeded` flag that propagates the break to the outer rule loop: previously only the per-rule pre-check existed; the inner regex pass could exhaust CPU without any escape |
| PL-CPU | MetaMapperDescribeCache | Added `&& !fallbackCpuReported` to the outer batch loop condition in `preloadFieldMaps`: previously the CPU `break` only escaped the inner fallback loop; the outer loop continued to the next 100-entity chunk, restarting fallback work after CPU was already exhausted |
| TRIM | CustomFieldHandler / ApexClassHandler | Added `.trim()` before all CMT text-field map lookups: SOQL IN returns rows with trailing spaces that pass the WHERE filter but fail the exact-match Apex Map lookup, silently dropping those dependencies |
| FLOOR | SupplementalResult | Fixed `appendErrorsSafe` floor to account for both truncation notices: `PRE_TRUNCATION_NOTICE_LENGTH` constant (16) added; floor raised to `Math.max(safeThreshold, TRUNCATION_NOTICE_LENGTH + PRE_TRUNCATION_NOTICE_LENGTH + 4)` = minimum 54; the prior floor of 40 allowed the combined 50-char notice pair to overflow `maxLength` |
| DEDUP | SupplementalResult | Pre-processed `safeExisting` into `Set<String> existingBaseMsgs` once before the error loop: the prior `safeExisting.contains(baseMsg)` call was O(existing_length) per error - at 500 errors × 131K-char log that is ~65M character comparisons and a real CPU timeout risk; Set lookup is O(1) |
...
public class SupplementalResult {
    ...
    // Length of the pre-truncation notice prepended to an already-overflowed existing log.
    // '\n[PRE-TRUNCATED]' = 16 chars.
    // FLOOR fix: the safeLimit floor must reserve headroom for BOTH notices because the
    // worst-case scenario appends PRE_TRUNCATION (16) + TRUNCATION (34) = 50 chars beyond
    // safeLimit. The prior floor of TRUNCATION_NOTICE_LENGTH + 6 = 40 was 10 chars short,
    // allowing a STRING_TOO_LONG crash when safeThreshold was passed below 50.
    private static final Integer PRE_TRUNCATION_NOTICE_LENGTH = 16;
    ...
    public String appendErrorsSafe(String existing, Integer maxLength, Integer safeThreshold) {
        ...
        // N1: null guard for maxLength - a null maxLength throws NullPointerException on the
        // subtraction below. safeThreshold was guarded (M1) but maxLength was not. Default 131072.
        if (maxLength == null) {
            maxLength = 131072;
        }
        // FLOOR: floor must accommodate both notices (PRE_TRUNCATION 16 + TRUNCATION 34 = 50).
        // Prior floor of TRUNCATION_NOTICE_LENGTH + 6 = 40 was 10 chars short, allowing
        // a STRING_TOO_LONG crash if both notices were appended to a near-limit log.
        Integer safeLimit = maxLength - Math.max(
            safeThreshold,
            TRUNCATION_NOTICE_LENGTH + PRE_TRUNCATION_NOTICE_LENGTH + 4
        );
        ...
        // per error message. At 500 unique errors × 131K-char log that is ~65M char comparisons,
        ...
        if (String.isNotBlank(safeExisting)) {
            for (String line : safeExisting.split('\n')) {
                // Only lines from this system carry the [Supplemental] marker.
                String b = line.substringAfter('[Supplemental] ');
...
public without sharing class MetaMapperDescribeCache {
    ...
    public static List<EntityDefinition> getCmtEntities() {
        if (cachedCmtEntities == null) {
            ...
            // L2D: surface the truncation if the org has >2000 CMT types so admins see it
            // in Error_Status_Message__c. Without this, excess entities are silently dropped.
            if (cachedCmtEntities.size() == 2000) {
                describeFailed.add(
                    'MetaMapperDescribeCache.getCmtEntities: query capped at 2000 CMT types; '
                    + 'additional CMT types in this org were not scanned. '
                    + 'Supplemental CMT dependencies from those types may be missing.'
                );
            }
        }
    ...
    public static void preloadFieldMaps(List<String> entityNames) {
        ...
        // PL-CPU: outer loop stops when fallbackCpuReported is set by the inner fallback loop.
        // Prior code only broke the inner per-entity loop on CPU exhaustion; the outer chunk
        // loop continued to the next batch, restarting expensive fallback work after CPU was
        // already exhausted. Using the existing flag as the stop signal avoids a new variable.
        Integer batchSize = 100;
        for (Integer i = 0; i < toDescribe.size() && !fallbackCpuReported; i += batchSize) {
...
public without sharing class CustomFieldHandler implements IDependencyTypeHandler {
    ...
    public SupplementalResult findSupplemental(
        ...
        SupplementalResult cmtResult  = findCmtRecordReferences(jobId, nodeByFieldName, nodeByFullName, nodeByLowerValue);
    ...
    private SupplementalResult findValidationRuleReferences(
        ...
        try {
            ...
            // VR-CPU: flag propagates a CPU exhaustion detected inside the field loop up to the
            // outer rule loop. The per-rule pre-check (before tokenization) only guards entry
            // into each rule; it cannot stop exhaustion during the inner O(#fields) regex pass.
            Boolean vrCpuExceeded = false;

            for (SObject rule : rules) {
                if (vrCpuExceeded) { break; }
                ...
                for (String lowerField : fieldApiNamesLower) {
                    // VR-CPU: regex .matcher.find() inside this loop can spike CPU in batches
                    // with many fields. Break and propagate to the outer rule loop via the flag.
                    if ((Decimal) Limits.getCpuTime() / Limits.getLimitCpuTime() >= CPU_THRESHOLD) {
                        String innerCpuMsg = 'CustomFieldHandler.findValidationRuleReferences: '
                            + 'CPU budget exceeded inside field loop; stopping. '
    ...
    // D1: removed dead `nodes` parameter (L3 moved all reads to findSupplemental; parameter was unused).
                ...
                for (Integer fi = 0; fi < relevantFields.size() && !cpuExceeded; fi += fieldBatchSize) {
                    ...
                    for (SObject record : records) {
                        ...
                        for (String field : fieldBatch) {
                            ...
                            // TRIM: SOQL IN is case-insensitive but does not strip trailing spaces.
                            // A database value of "Account.My_Field__c " passes the WHERE filter
                            // but fails the exact Apex Map lookup without trim(), silently dropping
                            // the dependency. trim() normalizes both leading and trailing whitespace.
            ...
            for (Integer fi = 0; fi < relevantFields.size() && !cpuExceeded; fi += fieldBatchSize) {
                ...
                for (SObject record : records) {
                    ...
                    for (String field : fieldBatch) {
                        ...
                        // TRIM: same whitespace-normalisation fix as CustomFieldHandler - SOQL IN
                        // passes trailing-space values that fail exact Apex Map lookup without trim().
                        List<Metadata_Dependency__c> parentNodes = nodeByClassName.get(value.trim().toLowerCase());
                        if (parentNodes == null) {
                            continue;
                        }
