...
export default class MetaMapperProgress extends LightningElement {
  ...
  set job(val) {
    ...
    if (
      ...
      this._elapsedFrozenSeconds = Math.floor(
        (Date.now() - new Date(val.CreatedDate).getTime()) / 1000
      );
  ...
  handleStatusEvent(eventData) {
    ...
    if (
      eventData &&
      eventData.streamingQuotaLimitExceeded &&
      !this._streamingQuotaBannerDismissed
    ) {
      this.showStreamingQuotaBanner = true;
    }
    if (
      eventData &&
      eventData.Status__c === "Cancelled" &&
    ...
    if (
      eventData &&
      ["Completed", "Failed", "Cancelled"].includes(eventData.Status__c)
    ) {
    ...
    if (
      eventData &&
      eventData.Status__c === "Processing" &&
      this.showPollingNotice
    ) {
      this._startPolling();
    }
  }
  ...
  get showCancelButton() {
    return (
      !this.isTerminal && !this.isPaused && this._cancelPhase !== "cancelled"
    );
  }
  get cancelButtonDisabled() {
    return this.cancelDisabled || this.showTimeoutBanner;
  }
  ...
  get resumeCurrentActive() {
    return this.resumeLoading && !this.resumeSlowerActive;
  }
  ...
  get pauseBannerText() {
    ...
    const apiName =
      this.job && this.job.Target_API_Name__c
        ? this.job.Target_API_Name__c
        : "the component";
  ...
  _effectiveBatchSize() {
    ...
    if (this.job && this.job.Batch_Size_Override__c)
      return this.job.Batch_Size_Override__c;
    return 50;
  }
  ...
  get statusLabel() {
    ...
    switch (this.status) {
      ...
      case "Cancelled":
        return `Analysis of ${name} cancelled. Partial results are available.`;
      default:
        return "";
    }
  }
  ...
  get failedErrorLog() {
    return (this.job && this.job.Scan_Diagnostic_Log__c) || "";
  }
  get hasFailedErrorLog() {
    return !!this.failedErrorLog;
  }
  get failedErrorToggleLabel() {
    return this.showFullError ? "Hide full error" : "View full error";
  }
  ...
  _resetPeWatchdog() {
    ...
    if (
      this.isTerminal ||
      this._isPeSuppressionActiveProp ||
      this._pollTimer ||
      !this._isMounted
    )
      return;
  ...
  handleCancelModalKeyDown(event) {
    if (event.key === "Escape") {
      event.stopPropagation();
      this.handleKeepRunning();
    }
  }
  ...
  handleStartNew() {
    this.dispatchEvent(
      new CustomEvent("startnewscan", { bubbles: true, composed: true })
    );
  }
