...
export default class MetaMapperSearch extends LightningElement {
  ...
  get activeTypeaheadOptionId() {
    if (
      this._focusedTypeaheadIdx < 0 ||
      !this.typeaheadResults[this._focusedTypeaheadIdx]
    )
      return null;
  ...
  get isSubmitDisabled() {
    ...
    const apiNameInput = this.template.querySelector(
  ...
  handleActiveFlowsChange(event) {
    this.isActiveFlowsOnly = event.detail.checked;
  }
  ...
  handleTargetObjectBlur() {
    clearTimeout(this._blurCloseTimer);
    this._blurCloseTimer = setTimeout(() => {
      this.typeaheadOpen = false;
  ...
  handleTypeaheadKeydown(event) {
    ...
    if (key === "ArrowDown") {
      ...
      this._setTypeaheadFocus(
        Math.min(
          this._focusedTypeaheadIdx + 1,
          this.typeaheadResults.length - 1
        )
      );
    } else if (key === "ArrowUp") {
      event.preventDefault();
      ...
      if (
        ...
        event.preventDefault();
        this.targetObject =
          this.typeaheadResults[this._focusedTypeaheadIdx].value;
        this.typeaheadOpen = false;
        this.targetObjectError = "";
        this._resetTypeaheadFocus();
      }
  ...
  _setTypeaheadFocus(idx) {
    ...
    this.typeaheadResults = this.typeaheadResults.map((r, i) => ({
      ...r,
      isFocused: i === idx
    }));
  }
  ...
  _resetTypeaheadFocus() {
    ...
    if (this.typeaheadResults.some((r) => r.isFocused)) {
      this.typeaheadResults = this.typeaheadResults.map((r) => ({
        ...r,
        isFocused: false
      }));
    }
  ...
  async _runTypeahead() {
    ...
    try {
      ...
      this.typeaheadResults = (results || []).map((r, i) => ({
        label: r.QualifiedApiName,
        value: r.QualifiedApiName,
        optionId: `typeahead-option-${i}`,
        isFocused: false
      }));
      this._focusedTypeaheadIdx = -1;
  ...
  async handleSubmit() {
    ...
    try {
      const jobId = await createJob({
        ...
        isActiveFlowsOnly: this.isActiveFlowsOnly
      });
      this.dispatchEvent(
  ...
  async handleViewRunningScan() {
    ...
    try {
      ...
      if (activeId) {
        this.dispatchEvent(
          new CustomEvent("viewrunningscan", {
            detail: { jobId: activeId },
            bubbles: true,
            composed: true
          })
        );
        ...
        this.dispatchEvent(
          new CustomEvent("showtoast", {
            detail: {
              message:
                "The running scan isn't visible to your account. It may belong to another user or have just completed. Try starting a new scan - if one is still running you will see this message again.",
              variant: "info"
            },
            bubbles: true,
            composed: true
