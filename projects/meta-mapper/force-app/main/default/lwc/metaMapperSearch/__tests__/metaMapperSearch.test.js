...
jest.mock(
  "@salesforce/apex/DependencyJobController.createJob",
  () => ({ default: jest.fn() }),
  { virtual: true }
);
jest.mock(
  "@salesforce/apex/DependencyJobController.getObjectList",
  () => ({ default: jest.fn() }),
  { virtual: true }
);
jest.mock(
  "@salesforce/apex/DependencyJobController.getComponentCount",
  () => ({ default: jest.fn() }),
  { virtual: true }
);
jest.mock(
  "@salesforce/apex/DependencyJobController.getActiveJobId",
  () => ({ default: jest.fn() }),
  { virtual: true }
);
...
describe("c-meta-mapper-search", () => {
  ...
  describe("form validation", () => {
    it("disables submit when no type or API name is entered", () => {
      const element = createElement("c-meta-mapper-search", {
        is: MetaMapperSearch
      });
    ...
    it("remains disabled for CustomField type until Target Object is populated", async () => {
      ...
      expect(submitButton(element).disabled).toBe(true);
      expect(
        element.shadowRoot.querySelector(".typeahead-container input")
      ).not.toBeNull();
    });
    ...
    it("shows the ValidationRule-specific help text only when ValidationRule is selected", async () => {
      ...
      expect(element.shadowRoot.textContent).not.toContain(
        "do not include the parent object"
      );
  ...
  describe("createJob() invocation", () => {
    ...
    it("surfaces a concurrency rejection message and sets isRunningScanError", async () => {
      createJob.mockRejectedValue({
        body: { message: "Another MetaMapper scan is already running." }
      });
      const element = createElement("c-meta-mapper-search", {
        is: MetaMapperSearch
      });
      ...
      expect(submitButton(element).disabled).toBe(false);
      expect(element.shadowRoot.textContent).toContain(
        "Another MetaMapper scan is already running."
      );
      expect(
        element.shadowRoot.querySelector("a.slds-m-left_x-small")
      ).not.toBeNull();
    });
