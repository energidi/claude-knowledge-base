...
export default class MetaMapperResults extends LightningElement {
  ...
  get showCopilotButton() {
    return this.copilotEnabled && !this.isMobile;
  }
  get showCopilotNotAvailable() {
    return (
      this._copilotChecked &&
      !this.copilotEnabled &&
      !this._copilotException &&
      !this.isMobile
    );
  }
  ...
  async loadResults() {
    ...
    try {
      ...
      if (this._pendingNodeId) {
        if (this.nodeMap.has(this._pendingNodeId)) {
          ...
          this.dispatchEvent(
            new CustomEvent("showtoast", {
              ...
              detail: {
                ...
                message:
                  "The linked component could not be found in this scan result.",
                variant: "warning"
              }
            })
          );
  ...
  _scheduleStatsAnnouncement() {
    ...
    this._statsAnnounceTimer = setTimeout(() => {
      ...
      region.textContent =
        "Type counts updated: " +
        this.typeCounts.map((tc) => `${tc.label} (${tc.count})`).join(", ");
    }, 150);
  }
  ...
  get isTreeTab() {
    return this.activeTab === "tree";
  }
  get showTreeLoadError() {
    return this.tabLoadFailed && this.activeTab === "tree";
  }
  get showGraphLoadError() {
    return this.tabLoadFailed && this.activeTab === "graph";
  }
  get isCompleted() {
  ...
  get isZeroResults() {
    return (
      ...
      !this.isSerializerFailure &&
      !this.isMidwayFailure
    );
  }
  get showTabs() {
  ...
  get isMidwayFailure() {
    return (
      this.job && this.job.Status__c === "Failed" && !this.isSerializerFailure
    );
  }

  get midwayFailureLog() {
  ...
  get midwayFailureToggleLabel() {
    return this.showMidwayFullError ? "Hide full error" : "View full error";
  }

  toggleMidwayFullError() {
    this.showMidwayFullError = !this.showMidwayFullError;
  }
  ...
  get availableTypes() {
    return extractTypes(this.allNodes);
  }
  get maxDepthValue() {
    return maxDepth(this.allNodes);
  }
  ...
  get summaryTruncated() {
    return (
      this.summaryText && this.summaryText.length > (this.isMobile ? 200 : 300)
    );
  }
  ...
  get isTransitioningStr() {
    return this.isTransitioning ? "true" : null;
  }
  get showStatsTileShimmer() {
    return (
      this.isCompleted &&
      this.job &&
      !this.job.Component_Type_Counts__c &&
      this.typeCounts.length === 0
    );
  }
  ...
  toggleSummaryExpandedKeyDown(event) {
    if (event.key === "Enter" || event.key === " ") {
      event.preventDefault();
      this.toggleSummaryExpanded(event);
    }
  }
  dismissSummary() {
    this.summaryDismissed = true;
  }
  dismissSummaryKeyDown(event) {
    if (event.key === "Enter" || event.key === " ") {
      event.preventDefault();
      this.dismissSummary(event);
    }
  }
        ...
        this.dispatchEvent(
          new CustomEvent("showtoast", {
            detail: {
              title: "Error",
              message:
                "Could not copy to clipboard. Your browser may require clipboard permission. Select and copy the text manually instead.",
              variant: "error"
            },
            bubbles: true,
            composed: true
  ...
  handleAskCopilot() {
    this.dispatchEvent(
      new CustomEvent("askcopilot", {
        detail: { summaryText: this.summaryText },
        bubbles: true,
        composed: true
      })
    );
  ...
  reloadResultsKeyDown(event) {
    if (event.key === "Enter" || event.key === " ") {
      event.preventDefault();
      this.reloadResults(event);
    }
  }
  handleStartNew() {
    this.dispatchEvent(
  ...
  handleExportPartialKeyDown(event) {
    if (event.key === "Enter" || event.key === " ") {
      event.preventDefault();
      this.handleExportPartial(event);
    }
  }

  handleSwitchToTree() {
    this._activateTab("tree");
  }
  handleRetryTab() {
