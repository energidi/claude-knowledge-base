...
function makeElement() {
    ...
    el.job = { Status__c: 'Completed', Target_API_Name__c: 'MyClass', Components_Analyzed__c: 2 };
...
describe('c-meta-mapper-results', () => {
    ...
    it('loads results via getNodeHierarchy on connect and renders the tab set', async () => {
        ...
        expect(getNodeHierarchy).toHaveBeenCalledWith({ jobId: 'a001000000000001AAA' });
    ...
    it('renders the error state and does not render tabs when getNodeHierarchy rejects', async () => {
        getNodeHierarchy.mockRejectedValue({ body: { message: 'The dependency data could not be loaded.' } });
        const el = makeElement();
        await flushPromises();

        expect(el.shadowRoot.querySelector('lightning-tabset')).toBeNull();
        expect(el.shadowRoot.textContent).toContain('The dependency data could not be loaded.');
    });
    ...
    describe('isTransitioning gating', () => {
        it('drops a node selection event received while a tab transition is in flight', async () => {
            ...
            graphEl.dispatchEvent(new CustomEvent('nodeselected', { detail: { nodeId: 'a001000000000002AAA' } }));
            await Promise.resolve();

            const panel = el.shadowRoot.querySelector('c-meta-mapper-component-details-panel');
    ...
    describe('filter reconciliation on load', () => {
        it('discards stale filter types not present in the new scan and dispatches filtersreset', async () => {
            ...
            const stored = JSON.parse(sessionStorage.getItem('metaMapper_filters_v1'));
