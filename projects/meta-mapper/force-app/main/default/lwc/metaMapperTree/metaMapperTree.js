...
export default class MetaMapperTree extends LightningElement {
  ...
  _buildChildrenMap(allNodes) {
    ...
    map.forEach((children) => {
      children.sort((a, b) => {
        ...
        return (a.Metadata_Name__c || "").localeCompare(
          b.Metadata_Name__c || ""
        );
      });
    });
  ...
  _makeRow(n, filteredIds) {
    ...
    const childList = (this._childrenMap.get(n.Metadata_Id__c) || []).filter(
      (c) => filteredIds.has(c.Metadata_Id__c)
    );
  ...
  _buildNameSegments(label) {
    ...
    segments.push({
      key: "seg-match",
      text: label.slice(idx, idx + term.length),
      isMatch: true
    });
    if (idx + term.length < label.length) {
      segments.push({
        key: "seg-post",
        text: label.slice(idx + term.length),
        isMatch: false
      });
    }
    return segments;
  ...
  _computeWindow() {
    ...
    const container =
      this.template && this.template.querySelector(".tree-scroll-container");
    const scrollTop = container ? container.scrollTop : 0;
    const clientHeight = container ? container.clientHeight : 450;
    const start = Math.max(0, Math.floor(scrollTop / ROW_HEIGHT) - BUFFER);
    const end = Math.min(
      total,
      Math.ceil((scrollTop + clientHeight) / ROW_HEIGHT) + BUFFER
    );
  ...
  get visibleRows() {
    return this._flatRows
      .slice(this._startIndex, this._endIndex)
      .map((row, i) => {
  ...
  handleKeyDown(event) {
    ...
    if (
      ...
      if (row) {
        const rowEl = this.template.querySelector(
          `[data-node-id="${row.Metadata_Id__c}"]`
        );
    ...
    if (key === "ArrowDown") {
      ...
      if (row) {
        if (row.hasChildren && row.isExpanded) {
          ...
          if (parentId) {
            const parentIdx = this._flatRows.findIndex(
              (r) => r.Metadata_Id__c === parentId
            );
            if (parentIdx >= 0) {
  ...
  _focusActiveRow() {
    ...
    Promise.resolve().then(() => {
      const el =
        this.template &&
        this.template.querySelector(`[data-node-id="${row.Metadata_Id__c}"]`);
      if (el) el.focus();
    });
  }
  ...
  handleViewInGraph() {
    ...
    if (!node) return;
    this.dispatchEvent(
      new CustomEvent("graphpathrequest", {
