...
export default class MetaMapperComponentDetailsPanel extends LightningElement {
  ...
  get breadcrumbs() {
    const cacheKey =
      this.selectedNodeId + ":" + (this.nodeMap ? this.nodeMap.size : 0);
  ...
  get setupUrl() {
    ...
    const node = this.selectedNode;
    const result =
      !node || !this.orgId ? null : resolveSetupUrl(node, this.orgId);
  ...
  get showConfidenceBadge() {
    ...
    return !!(
      n &&
      n.Discovery_Source__c === "Supplemental" &&
  ...
  get confidenceBadgeClass() {
    return this.showConfidenceWarning
      ? "confidence-badge confidence-low"
      : "confidence-badge confidence-ok";
  }
