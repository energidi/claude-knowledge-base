...
const LOW_CONTRAST_HIGHLIGHT_TYPES = new Set([
  "ApexClass",
  "ApexTrigger",
  "CustomField",
  "WorkflowRule"
...
export default class MetaMapperGraph extends LightningElement {
    ...
    try {
      ...
      this._chart.on("click", (params) => {
        ...
        if (this._isMobileState) {
          if (this._showMobileGraphTip) {
            this._showMobileGraphTip = false;
            try {
              sessionStorage.setItem("metaMapper_mobileGraphTip_v1", "true");
            } catch {
              /* ignore */
            }
            return;
      ...
      this._chart.on("contextmenu", (params) => {
        ...
        if (params.dataType === "node") {
          ...
          setTimeout(() => {
            if (!this._isMounted) return;
            const first = this.template.querySelector(
    ...
    this._handleCtrlK = (e) => {
      if (e.ctrlKey && e.key === "k") {
        ...
        const activeNodeId =
          ...
          this._activeNodeIndex < this._orderedNodeIds.length
  ...
  _getVisibleNodes() {
    ...
    if (this._collapsedNodes.size === 0) {
      return filtered.filter(
        (n) => (n.Dependency_Depth__c || 0) <= this._maxVisibleDepth
      );
    }
  ...
  _computeCollapseHiddenSet(filtered) {
    ...
    const sorted = filtered
      .slice()
      .sort(
        (a, b) => (a.Dependency_Depth__c || 0) - (b.Dependency_Depth__c || 0)
      );
    sorted.forEach((n) => {
      const parentId = n.Parent_Dependency__c;
      if (
        parentId &&
        (this._collapsedNodes.has(parentId) || hidden.has(parentId))
      ) {
  ...
  _renderGraph() {
    ...
    this._nodeIdToDataIndex = new Map(
      visible.map((n, i) => [n.Metadata_Id__c, i])
    );
  ...
  _scheduleAriaTableRebuild(visible) {
    ...
    this._ariaRebuildTimer = setTimeout(() => {
      ...
      this._ariaTableRows = visible.map((n) => ({
        ...
        flags: [
          n.Is_Circular__c && "Circular",
          n.Is_Dynamic_Reference__c && "Dynamic"
        ]
          .filter(Boolean)
          .join(", ")
      }));
  ...
  static _getBadgeEntries(node) {
    ...
    if (lowConfidence) {
      entries.push({
        ...
        text: MetaMapperGraph._confidencePopoverText(
          node.Supplemental_Confidence__c
        )
      });
    }
    if (node.Is_Dynamic_Reference__c) {
      entries.push({
        ...
        glyph: "[!]",
        label:
          "Warning: dynamic Apex string reference, cannot be statically resolved",
    ...
    if (node.Discovery_Source__c === "Supplemental" && !lowConfidence) {
      entries.push({
        ...
        glyph: "[S]",
        label:
          "Supplemental match: found through secondary analysis, not the standard Tooling API",
  ...
  _buildOption(visibleNodes) {
    ...
    const edges = visibleNodes
      .filter(
        (n) => n.Parent_Dependency__c && idSet.has(n.Parent_Dependency__c)
      )
      .map((n) => ({
        source: n.Parent_Dependency__c,
        target: n.Metadata_Id__c
      }));
    ...
    const echartsNodes = visibleNodes.map((n) => {
      ...
      if (
        n.Supplemental_Confidence__c != null &&
        n.Supplemental_Confidence__c < 70
      ) {
        flags.push(
          `${n.Supplemental_Confidence__c}% confidence - verify manually`
        );
      }
      ...
      const badgeGlyph = badges.length
  ...
  _activateFocusPath(nodeId) {
    ...
    this._announceAriaLive(
      `Focus path activated: ${pathSet.size} nodes highlighted`
    );
    setTimeout(() => {
      if (!this._isMounted) return;
  ...
  _handleSearchChange(term) {
    ...
    if (!term) {
      ...
      this._searchHighlights = new Set(
        visible
          .filter((n) =>
            (n.Metadata_Name__c || "").toLowerCase().includes(lower)
          )
          .map((n) => n.Metadata_Id__c)
      );
      this._announceAriaLive(
        `${this._searchHighlights.size} nodes match your search`
      );
    }
    this._renderGraph();
  }
  ...
  get showShortcutLegendButton() {
    return !this.isMobile;
  }
  get showMobileGraphTip() {
    return this._showMobileGraphTip;
  }
  ...
  get legendItems() {
    return LEGEND_TYPES.map((item) => ({
      ...
      color:
        item.type === "_default"
          ? DEFAULT_COLOR
          : TYPE_COLORS[item.type] || DEFAULT_COLOR,
  ...
  get ctxCollapseLabel() {
    ...
    if (node && (node.Dependency_Depth__c || 0) === 0)
      return "Collapse all children";
    ...
    if (visible.length > 1000) {
      ...
      setTimeout(() => {
        ...
        const btn = this.template.querySelector(
          ".modal-footer .slds-button_neutral"
        );
        if (btn) btn.focus();
      }, 0);
    } else {
  ...
  handleSwitchToTree() {
    this.dispatchEvent(
      new CustomEvent("switchtotree", { bubbles: true, composed: true })
    );
  }
  ...
  closeContextMenu() {
    ...
    if (lastMenu) {
      setTimeout(() => {
        ...
        const target =
          savedFocus && document.contains(savedFocus)
            ? savedFocus
  ...
  handleCtxMenuKeyDown(e) {
    if (e.key === "Escape") {
      ...
      const items = Array.from(
        this.template.querySelectorAll('.ctx-menu [role="menuitem"]')
      );
  ...
  _moveVirtualFocus(delta) {
    ...
    if (this._chart) {
      const prevDataIndex =
        prevNodeId != null
          ? this._nodeIdToDataIndex.get(prevNodeId)
          : undefined;
