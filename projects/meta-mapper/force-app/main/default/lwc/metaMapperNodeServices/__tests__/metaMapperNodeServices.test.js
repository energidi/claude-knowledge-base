...
describe("c-meta-mapper-node-services", () => {
  ...
  describe("resolveSetupUrl", () => {
    ...
    it("returns the Apex Classes setup URL for ApexClass", () => {
      ...
      expect(resolveSetupUrl(node, orgId)).toBe(
        `/${orgId}/lightning/setup/ApexClasses/home`
      );
    });

    it("returns the Apex Classes setup URL for ApexTrigger", () => {
      const node = {
        Metadata_Type__c: "ApexTrigger",
        Metadata_Id__c: "01q000000000001"
      };
      expect(resolveSetupUrl(node, orgId)).toBe(
        `/${orgId}/lightning/setup/ApexClasses/home`
      );
    ...
    it("returns the Flow Builder URL with flowId embedded for Flow", () => {
      const node = {
        ...
        Metadata_Id__c: "301000000000001"
      };
      const url = resolveSetupUrl(node, orgId);
      expect(url).toBe(
        `/${orgId}/builder_platform_interaction/flowBuilder.app?flowId=301000000000001`
      );
    ...
    it("returns the Workflow Rules home URL for WorkflowRule", () => {
      const node = {
        Metadata_Type__c: "WorkflowRule",
        Metadata_Id__c: "04b000000000001"
      };
      expect(resolveSetupUrl(node, orgId)).toBe(
        `/${orgId}/lightning/setup/WorkflowRules/home`
      );
    });

    it("returns the Report view URL for Report", () => {
      const node = {
        Metadata_Type__c: "Report",
        Metadata_Id__c: "00O000000000001"
      };
      expect(resolveSetupUrl(node, orgId)).toBe(
        `/${orgId}/lightning/r/Report/00O000000000001/view`
      );
    });
    ...
    it('falls back to splitting Metadata_Name__c on "." for CustomField when no parentObject in context', () => {
      const node = {
        Metadata_Type__c: "CustomField",
        Metadata_Name__c: "Contact.My_Field__c"
      };
      expect(resolveSetupUrl(node, orgId)).toBe(
        `/${orgId}/lightning/setup/ObjectManager/Contact/FieldsAndRelationships/view`
      );
    });

    it("returns the Validation Rules view URL for ValidationRule", () => {
      const node = {
        Metadata_Type__c: "ValidationRule",
        Metadata_Name__c: "Opportunity.Validate_Phone"
      };
      expect(resolveSetupUrl(node, orgId)).toBe(
        `/${orgId}/lightning/setup/ObjectManager/Opportunity/ValidationRules/view`
      );
    });
  ...
  describe("isNamespacePrefixed", () => {
    ...
    it("includes a plain custom field with only the standard __c suffix", () => {
      expect(isNamespacePrefixed("My_Custom_Field__c", "CustomField")).toBe(
        false
      );
    });
  ...
  describe("maxDepth", () => {
    ...
    it("returns the deepest Dependency_Depth__c value", () => {
      const nodes = [
        ...
        { Dependency_Depth__c: 4 },
        { Dependency_Depth__c: 2 }
      ];
