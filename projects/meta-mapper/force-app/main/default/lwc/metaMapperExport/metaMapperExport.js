...
export default class MetaMapperExport extends LightningElement {
  ...
  _buildCsv(nodes) {
    ...
    const header =
      "Level,Metadata_Type,Metadata_Name,Metadata_ID,Parent_Name,Is_Circular,Is_Dynamic";
    const rows = nodes.map((n) => {
      const parent = n.Parent_Dependency__c
        ? nodeMap.get(n.Parent_Dependency__c)
        : null;
      return [
    ...
    this.dispatchEvent(
      new CustomEvent("showtoast", {
        ...
        detail: {
          title: "Export failed",
          message:
            "Export failed. Try filtering to fewer nodes first, or use JSON instead of CSV for large result sets.",
          variant: "error"
        }
      })
    );
