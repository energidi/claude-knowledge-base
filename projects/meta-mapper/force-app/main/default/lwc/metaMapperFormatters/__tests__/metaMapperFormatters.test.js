...
describe("c-meta-mapper-formatters", () => {
  ...
  describe("renderPills", () => {
    it('renders isWrite: true as "Writes to this field"', () => {
      expect(renderPills(JSON.stringify({ v: 1, isWrite: true }))).toBe(
        "Writes to this field"
      );
    });

    it('renders activeVersions as "N active versions"', () => {
      expect(renderPills(JSON.stringify({ v: 1, activeVersions: 3 }))).toBe(
        "3 active versions"
      );
    });
    ...
    it('renders cycleClosesAt as "Cycle closes at ..." text', () => {
      expect(
        renderPills(JSON.stringify({ v: 1, cycleClosesAt: "01p000000000001" }))
      ).toBe("Cycle closes at 01p000000000001");
    });
    ...
    it('renders a multi-item filterUsage array as a comma-joined "Used as:" list', () => {
      expect(
        renderPills(
          JSON.stringify({
            v: 1,
            filterUsage: ["filter", "grouping", "column"]
          })
        )
    ...
    it("renders a single-item filterUsage array in singular form with no trailing comma", () => {
      expect(
        renderPills(JSON.stringify({ v: 1, filterUsage: ["filter"] }))
      ).toBe("Used as: filter");
    });
    ...
    it('renders an unrecognized key as plain text "key: value" fallback', () => {
      expect(renderPills(JSON.stringify({ v: 1, someNewKey: "X" }))).toBe(
        "someNewKey: X"
      );
    });
  });
  ...
  describe("sanitizeFilename", () => {
    ...
    it('replaces "/" and "\\" with "_"', () => {
      expect(sanitizeFilename("Account/Phone\\Field__c")).toBe(
        "Account_Phone_Field__c"
      );
    });
