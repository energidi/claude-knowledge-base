...
export default class MetaMapperApp extends LightningElement {
  ...
  async _handleHealthCheckPassed() {
    ...
    if (deepJobId) {
      ...
      try {
        ...
        this.view = "search";
        this._showToast(
          "Could not load this scan result. Check your connection and try again.",
          "error"
        );
      } finally {
    ...
    if (!localStorage.getItem(TOUR_KEY) && this.view === "search") {
      ...
      setTimeout(() => {
        const modal = this.template.querySelector(
          'section[aria-label="MetaMapper first-time tour"]'
        );
  ...
  _handleSubscribeFailure(err) {
    ...
    const isQuotaLimit =
      msg &&
      /concurrent clients limit exceeded|streaming api.*limit/i.test(msg);
    const prog = this.template.querySelector("c-meta-mapper-progress");
    if (prog) {
      prog.handleStatusEvent({
        isPeSuppressionActive: true,
        streamingQuotaLimitExceeded: isQuotaLimit
      });
    }
  }
  ...
  _handlePEEvent(payload) {
    ...
    if (prog)
      prog.handleStatusEvent({
        ...payload,
        isPeSuppressionActive: this._isPeSuppressionActive
      });
    const res = this.template.querySelector("c-meta-mapper-results");
    if (res)
      res.notifyStatusChange({ ...(this.job || {}), Status__c: newStatus });
  }
  ...
  _storeJobResult(wrapper) {
    ...
    this._batchSizeInUse =
      wrapper.batchSizeInUse != null ? wrapper.batchSizeInUse : null;
    this._maxComponentsCap =
      wrapper.maxComponentsCap != null ? wrapper.maxComponentsCap : 0;
    this._retentionHours =
      wrapper.retentionHours != null ? wrapper.retentionHours : 72;
  }
  ...
  get maxComponentsCap() {
    return this._maxComponentsCap || 0;
  }
  get retentionHours() {
    return this._retentionHours || 72;
  }
  get showApp() {
    return (
  ...
  handleLearnMore() {
    ...
    setTimeout(() => {
      const modal = this.template.querySelector(
        'section[aria-label="MetaMapper setup instructions"]'
      );
      if (modal) modal.focus();
    }, 0);
  }
  ...
  closeTour() {
    ...
    if (window.innerWidth < 1024) {
      ...
      this._tourTriggerElement &&
      this._tourTriggerElement !== document.body
    ) {
