...
| `Is_Platform_Events_Auto_Suppressed__c` | Checkbox | Set to `true` by `DependencyNotificationService` when auto-suppression fires (org daily PE delivery limit >80% consumed). Read by `getJobStatus()` alongside the CMDT `Should_Disable_Platform_Events__c` flag to derive `peSuppressionActive` in the `JobStatusResult` wrapper - the LWC switches to polling mode immediately on receiving `peSuppressionActive: true`. The CMDT flag is the admin-controlled switch; this field is the runtime auto-suppression signal. Do not edit manually. |
...
| `Should_Disable_Platform_Events__c` | "Disable live progress updates" | "Turn on if your org is hitting real-time event limits. Progress will refresh every few seconds instead." |
...
// +1 per Flow node in current batch if Only_Include_Active_Flows__c = true (1 validation callout per Flow node)
...
// needsFlowValidation = Only_Include_Active_Flows__c AND current batch contains Flow nodes (count, not boolean - 1 callout per node)
...
| `Should_Disable_Platform_Events__c` | Checkbox | false | When true, suppresses `Dependency_Scan_Status__e` publish and falls back to polling via `getJobStatus()`. Use when org is approaching the daily Platform Event delivery limit. |
...
| `Only_Include_Active_Flows__c` | When checked, inactive Flow versions are excluded from scan results. Reduces scan scope and processing time. Default: true. Unchecking this includes deprecated Flow versions and may significantly increase scan duration. |
...
| `Should_Disable_Platform_Events__c` | When enabled, suppresses real-time progress events and the UI falls back to polling every few seconds. Use this if your org is approaching its daily Platform Event delivery limit. Can also be set automatically by the engine if the limit is exceeded. Default: false. |
