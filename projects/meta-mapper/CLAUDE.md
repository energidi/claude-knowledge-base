...
- **Circular node path:** Keep the **full `Ancestor_Path__c`** on circular nodes - do NOT set to null. The path is most valuable precisely when a cycle is found (debugging, export). Mark `Is_Circular__c = true`, `Has_Fetched_Dependencies__c = true`. Append the cycle-closing segment to `Dependency_Context__c` as `{"cycleClosesAt": "<parentMetadataId>"}` for UI visualization.
...
> **Ancestor_Path__c capacity and guard:** At 18 chars/ID + 1 delimiter, a depth-1,500 path would be ~28,500 chars - within the Long Text 32768 field limit. However, appending beyond ~32,000 chars risks a `StringException`. **Before building the path for any child node**, check: `if ((parent.Ancestor_Path__c?.length() ?? 0) + 20 > 32000)` - if true, set `Has_Fetched_Dependencies__c = true` and `Dependency_Context__c = '{"v":1,"maxDepthExceeded":true}'`, log to `Scan_Diagnostic_Log__c` ("Max ancestor depth exceeded at [nodeId]; traversal stopped at this node"), and insert the child (do not skip - it still needs a record). Do NOT set `Is_Circular__c = true`: depth overflow is a traversal ceiling, not an ancestry cycle. Setting `Is_Circular__c` would cause the UI to render these nodes with dashed "circular dependency" styling, which is incorrect. `Has_Fetched_Dependencies__c = true` is the correct mechanism to stop further traversal. Do NOT fail the job. The actual field limit is 32,768 chars; the 32,000 threshold provides 768 chars of conservative headroom against any concurrent path-append race. The authoritative check is `(parent.Ancestor_Path__c?.length() ?? 0) + 20 > 32000`, providing 768 chars of headroom against the 32,768 field limit.
...
> **Kill switch (`Max_Concurrent_Jobs__c = 0`):** `createJob()` checks `maxConcurrent <= 0` before the two-check concurrency guard and throws a dedicated message: "MetaMapper scans are currently disabled by your admin (Max Concurrent Scans is set to 0)." Without this explicit check, the concurrency guard below would reject every submission with the generic "another scan is already running" message, since `0 >= 0` is always true - which is misleading when the real cause is an admin-set 0. This is documented as the supported way to disable all new MetaMapper scans org-wide without a deployment.
...
- Permission Set `MetaMapper_Admin` grants Create/Read/Update (never Delete) on both custom objects, Named Credential principal access, and LWC/controller access. **No user-context delete path exists or is needed** - deletion is entirely engine-managed via SYSTEM_MODE Apex (ring buffer, nightly cleanup, cascade); granting `allowDelete` would let an assigned user manually delete job/node records mid-scan via Data Loader/Workbench, corrupting in-flight engine state (Round 81). **FLS requirement:** the permission set must explicitly grant Field-Level Security READ access to all `Metadata_Dependency__c` fields returned by `getNodeHierarchy()`. Engine-internal-only fields (`Ancestor_Id_Index__c`, `Unique_Component_Key__c`) must be excluded from the `getNodeHierarchy()` SOQL field list - these fields are not consumed by the LWC, and omitting them from the projection avoids `WITH USER_MODE` failures for non-sysadmin users who lack FLS on those fields. Every field explicitly assigned a value by `insert as user` / `update as user` in `DependencyJobController` (e.g. `Job_Type__c`, `Is_Active_Flows_Only__c`, `Status__c`) must also be granted `editable = true` here - a field left `editable = false` while the controller assigns it under USER_MODE throws an FLS exception for every non-System-Administrator user assigned this permission set.
- **`Dependency_Scan_Status__e` cross-user broadcast (accepted trade-off):** Platform Events have no row-level sharing, so `Dependency_Scan_Status__e` payloads are delivered via CometD to **every** subscribed MetaMapper user in the org, regardless of the target job's Private OWD on `Metadata_Scan_Job__c`. Two users each running their own scan will each receive the other's `Scan_Job_Id__c`, `Status__c`, and `Components_Analyzed__c` in the browser, even though `getJobStatus()` would deny them access to that job record. This is mitigated, not eliminated, by keeping the event payload generic: `Progress_Message__c` must never carry `Target_API_Name__c`, `Metadata_Name__c`, or any other metadata identifier - only a generic status phrase (e.g. "Analyzing metadata... N components found so far"). The LWC (`metaMapperApp.js`) discards events for jobs it does not own via a client-side `Scan_Job_Id__c` filter; this filtering is a UX convenience, not a security boundary. Do not add metadata names or diagnostic content to `Progress_Message__c` in `DependencyNotificationService.publishProgress()` - doing so would leak metadata identifiers cross-user via this channel.
...
**Null `Result_File_Id__c` guard:** If the evicted job's `Result_File_Id__c` is null (e.g. data corruption or a historical record predating the field), skip the `ContentDocument` delete step and proceed with job record deletion only. Append a diagnostic notice to the CURRENT job's `Scan_Diagnostic_Log__c` via `SupplementalScanResult.appendNoticeSafe()`: `'[timestamp] Ring buffer: evicted job [evictedJobId] had no result file; job record deleted, no ContentDocument to clean up.'` This is not an error - proceed normally.
...
| `Pause_Reason__c` | Picklist | Machine-readable reason for the most recent pause. Values: `ComponentLimitReached` (scan paused because `Components_Analyzed__c >= Max_Components__c`), `StallDetected` (scan paused because `Processing_Cycle_Count__c - Last_Progress_Cycle__c >= Stall_Pause_Threshold__c`). Null when `Status__c` is not `Paused`. The LWC reads this field to render the correct pause banner copy and resume options without parsing `Scan_Diagnostic_Log__c` strings. Set by the engine only; do not edit manually. |
| `Is_Platform_Events_Auto_Suppressed__c` | Checkbox | Set to `true` by `DependencyNotificationService` when auto-suppression fires (org daily PE delivery limit >80% consumed). Read by `getJobStatus()` alongside the CMDT `Is_Platform_Events_Disabled__c` flag to derive `isPeSuppressionActive` in the `JobStatusResult` wrapper - the LWC switches to polling mode immediately on receiving `isPeSuppressionActive: true`. The CMDT flag is the admin-controlled switch; this field is the runtime auto-suppression signal. Do not edit manually. |
...
|---|---|
| `MetadataScanJobSelector` | `getByIdForEngine(String jobId)` - minimal fields for engine; `getClosedJobsBefore(DateTime threshold)` - for cleanup; `countActiveQueueables()` - scoped `AsyncApexJob` count for concurrency guard; `getByIdForStatusView(String jobId)` - full UI fields WITH USER_MODE; `getByIdForResume(String jobId)` - status + flow fields WITH USER_MODE; `getByIdForSummary(String jobId)` - for ScanSummaryQueueable; `countActiveJobRecords()` - secondary soft lock for concurrency gate; `getStatusOnly(String jobId)` - reads Status__c only, used by cancellation and resume pre-checks; `getForFailedUpdateLocked(String jobId)` - FOR UPDATE variant to prevent concurrent failure handler races (called by `updateJobFailed()`; the sole `updateJobFailed()` selector path - the earlier unlocked variant was removed as dead code, Round 81); `getByIdForLock(String jobId)` - FOR UPDATE query used by ring buffer to serialize concurrent `ScanResultFileQueueable` completions; `getByIdForLogAppend(String jobId)` - reads `Scan_Diagnostic_Log__c` only for safe log-append operations; `getCompletedJobsOldestFirst(Integer lim)` - returns completed jobs ordered by `Status_Closed_At__c ASC, Id ASC` for ring buffer eviction; `getActiveJobId()` - returns the Id of the first Initializing or Processing job visible to the running user WITH USER_MODE; used by `DependencyJobController.getActiveJobId()` for concurrency rejection flow navigation |
...
| `DependencyFetchContext` | Mutable context object passed to `IMetadataDependencyService.fetchDependencies()`. Holds `jobId`, `isActiveFlowsOnly`, `maxResultCount` (max rows seen across chunks - drives `queryMorePossible` in the guardrail), accumulated `errors` (diagnostic strings written to `Scan_Diagnostic_Log__c` by the caller after all handlers complete), `hasQueryMoreFailed` (set by the service when a QueryMore cursor expires, prevents premature `Has_Fetched_Dependencies__c = true`), and `failedParentMetaIds` (parent IDs whose Tooling API fetch failed - excluded from the fully-processed set so they are re-queried on the next Queueable execution). Errors are accumulated rather than thrown so a single-batch failure does not abort the whole execution. |
| `SupplementalScanResult` | Return type for `IDependencyTypeHandler.findSupplemental()`. Encapsulates the list of discovered supplemental `Metadata_Dependency__c` nodes (ready for upsert by the caller) and diagnostic error strings (appended to `Scan_Diagnostic_Log__c` by `DependencyQueueable` after all handlers complete, via `appendErrorsSafe()` which enforces the 32,768-char log field limit). Errors are accumulated in-memory rather than persisted inside handlers, removing unbulkified DML from handler catch blocks and ensuring errors survive Queueable rollbacks. **Two permitted write paths to `Scan_Diagnostic_Log__c`:** (1) `appendErrorsSafe(existing, maxLength, safeThreshold)` - instance method used by `DependencyQueueable` to batch-append accumulated supplemental handler errors; (2) `SupplementalScanResult.appendNoticeSafe(existing, notice)` - static method used by engine, controller, and notification service for single diagnostic notice appends (e.g. PE suppression notices, truncation warnings, QueryMore cursor expiry notices). **Direct string concatenation to the log field is forbidden across ALL classes** (DependencyQueueable, DependencyNotificationService, ScanResultFileQueueable, DependencyCleanupBatch, and all handler classes). Every log write must use one of the two permitted paths above - both enforce the 32,768-char ceiling by truncating the new text so that the existing log content is preserved. |
| `MetadataTypeDescribeService` | Shared transaction-level cache for CMT describe data. Centralizes `Schema.describeSObjects()` calls so `CustomFieldDependencyHandler` and `ApexClassDependencyHandler` share a single describe result within the same Queueable execution, avoiding redundant CPU and heap cost from duplicate global describes. Must NOT be called from `@AuraEnabled` controllers or synchronous contexts - `Schema.describeSObjects()` is expensive and callers must verify remaining CPU budget before invoking. Cache lifetime is one Apex transaction (static fields reset on every Queueable execution). |
...
| `CustomFieldDependencyHandler` | Supplemental: WorkflowFieldUpdate (95), ValidationRule regex (65), FlexiPage XML (60), CMT lookups (85), Lookup relationships (95). **Regex safety rule:** all regex patterns must be non-backtracking (no nested quantifiers). Before each regex call, check `Limits.getCpuTime() / Limits.getLimitCpuTime() >= 0.60` - if true, skip the field, log a diagnostic notice to `Scan_Diagnostic_Log__c`, and continue. ValidationRule formula fields in complex orgs can be 10,000+ characters; unbounded backtracking patterns will hit the CPU limit and fail the Queueable. **CMT entity scan cap:** `MAX_CMT_ENTITIES_PER_EXECUTION = 30` bounds the number of CMT entities scanned per handler invocation in `findCmtRecordReferences()` - without this cap, `Database.query()` call count scales unboundedly with the org's CMT entity count, risking exhaustion of the 100-query SOQL budget in a single handler call on LDV orgs with many CMT types. Once the cap is reached, remaining entities are deferred (not scanned) and a diagnostic notice is logged. |
| `ApexClassDependencyHandler` | Supplemental: CMT references (85); flags `Is_Dynamic_Reference__c`. **CMT entity scan cap:** `scanCmtEntity()` is bounded by the same `MAX_CMT_ENTITIES_PER_EXECUTION = 30` constant and deferral/logging pattern as `CustomFieldDependencyHandler`, for the same reason. |
| `FlowDependencyHandler` | Supplemental: QuickActionDefinition, subflows, WebLink URLs |
| `DependencyQueueable` | Async engine. Constructor: `DependencyQueueable(String jobId, Boolean isActiveFlowsOnly, Integer overrideBatchSize)`. **Node cap check:** runs after the unprocessed-batch fetch and empty-batch exit check, and only when the batch is non-empty. A final execution where no unprocessed nodes remain hands off to `ScanResultFileQueueable` before either stall or cap checks fire - a scan completing exactly at `Max_Components__c` correctly transitions to Completed rather than Paused. When the batch IS non-empty: if `Components_Analyzed__c >= Max_Components__c` (and `Max_Components__c > 0`), transition job to `Status__c = 'Paused'`, set `Scan_Diagnostic_Log__c` with: "Scan paused: node limit of [N] reached. This protects your org's data storage. Raise Max_Components__c in MetaMapper Settings to continue." Do not self-chain. The LWC pause banner surfaces the resume option. Note: the cap is a soft ceiling - the final batch before the limit is reached may push `Components_Analyzed__c` up to `Max_Components__c + Scan_Batch_Size__c - 1`. - `overrideBatchSize` is null for normal execution; set to half of `Scan_Batch_Size__c` when `resumeJob()` triggers after a hot-loop pause. Savepoint/catch; cancel check; CMDT read via `IMetaMapperSettingsProvider`; **batch fetch** (`nextUnprocessed(jobId, batchSize)`); **Step 5a - root ID resolution (first execution only):** `createJob()` inserts the root `Metadata_Dependency__c` with blank `Metadata_Id__c` because the Tooling API ID is not known at job creation time. On the first Queueable execution, if any node in the unprocessed batch has a blank `Metadata_Id__c`, the engine calls `MetadataDependencyService.resolveRootId(type, apiName, parentObject)` to resolve the actual 18-char Tooling API ID and updates the root node before the main fetch begins. Only one root node per job; `break` after the first match; **empty-batch exit** (if `nextUnprocessed()` returned an empty list: re-verify `Status__c` to prevent enqueuing the serializer on a Cancelled job, then enqueue `ScanResultFileQueueable` and return - stall detection and node cap check are bypassed on this path); stall detection (Step 7, only reached when batch is non-empty); node cap check (Step 8, only reached when batch is non-empty); pre-batch + mid-loop seven-limit guardrail; scoped dedup + upsert by `Unique_Component_Key__c`; **upsert partial failure logging:** after each bulk upsert call, iterate `Database.UpsertResult[]` for errors (`!result.isSuccess()`), accumulate failure messages (format: `'Upsert failures ([N] nodes dropped): [errorSummary]'`), and append to `Scan_Diagnostic_Log__c` via `SupplementalScanResult.appendNoticeSafe()` - do NOT fail the job; treat dropped nodes as best-effort losses, as this surfaces silently-dropped nodes such as those rejected by the `Metadata_Id_Must_Be_18_Characters` validation rule; two-tier cycle detection; callouts; HTTP 414/431 reactive split-and-retry (halves batch, retries both halves as separate callouts, does not fail job - logs restart to `Scan_Diagnostic_Log__c`); handlers; one PE event per execution (suppressed if `Is_Platform_Events_Disabled__c`); self-chain. **On completion:** when no unprocessed nodes remain, instead of directly transitioning to Completed, enqueues `ScanResultFileQueueable` - this separates the concern of engine traversal (this class) from result persistence (serializer). The engine does not transition Status itself; `ScanResultFileQueueable` owns the Completed transition. **DML bulkification rule (critical):** child nodes discovered during the result-processing loop are accumulated in a `List<Metadata_Dependency__c>` and upserted in a single bulk statement after the loop completes (or before a mid-loop self-chain fires). Never upsert per-node inside the loop. |
| `DependencyNotificationService` (implements `IScanNotificationService`) | `publishProgress()` - one event per execution; checks org daily PE allocation via `OrgLimits` before publishing (auto-suppresses and flips `Is_Platform_Events_Disabled__c` if >80% consumed - appends suppression notice to `Scan_Diagnostic_Log__c` for admin visibility); `sendCompletionNotification()`. Does NOT enqueue `ScanSummaryQueueable` - that is the responsibility of `ScanResultFileQueueable` after the Completed transition. |
...
| `metaMapperFilters` | Pure utility module (no template, no Apex calls). Owns sessionStorage persistence and validation for shared filter state. Exports: `loadFilters` (reads from `sessionStorage` key `metaMapper_filters_v1`, falls back to `DEFAULT_FILTERS` on parse error), `saveFilters`, `validateFilters` (discards stored type values absent from the current scan's node set), `DEFAULT_FILTERS`. |
| `metaMapperNodeServices` | Pure utility module (no template, no Apex calls). Provides node-level operations shared across Tree and Graph views. Exports: `applyFilters`, `buildNodeMap`, `buildTypeCounts`, `extractTypes`, `maxDepth`, `resolveSetupUrl` (maps `Metadata_Type__c` to a Salesforce Setup URL; reads `parentObject` from `Dependency_Context__c`), `isNamespacePrefixed` (namespace prefix detection for package.xml export exclusion). |
| `metaMapperFormatters` | Pure utility module (no template, no Apex calls). Text and time formatting helpers shared across components. Exports: `formatElapsed` (seconds to MM:SS / H:MM:SS), `truncateAt` (word-boundary truncation), `renderPills` (converts `Dependency_Context__c` JSON keys to plain-English tooltip sentences). |
...
- **Right-click:** context menu with "Copy API Name", "Focus path to root", "Collapse subtree". **"Focus path to root" in the right-click menu operates on the right-clicked node, NOT the left-click selected node** - right-click does not change selection. The Node Details Panel is NOT updated by right-click. When right-clicking while focus mode is active, the current focus path clears **synchronously before the menu renders** (canvas undims before menu appears). If the right-clicked node is dimmed (non-focused), focus clears before the context menu appears. When "Focus path to root" is selected from the context menu, keyboard focus moves to the "Clear Focus" button in the toolbar after the menu closes - identical to direct focus path activation. This ensures focus mode is immediately dismissible by keyboard without additional Tab presses. **Context menu dismiss:** `role="menu"` on the context menu container. `role="menuitem"` on each item. When the context menu opens via any method (mouse right-click or keyboard), focus moves programmatically to the first menu item. **Keyboard trigger:** **Shift+F10** (or the platform Menu key) opens the context menu on the currently focused virtual-focus node in the graph canvas. This is the keyboard alternative to mouse right-click, required by WCAG 2.1.1. The keyboard listener is scoped to the graph canvas wrapper element (same as Ctrl+K) and fires only when `event.target` is a descendant of the canvas wrapper. (a) Esc closes the menu and returns focus to the element that had focus immediately before the context menu opened - tracked via `this._lastFocusBeforeMenu = document.activeElement` captured on the `contextmenu` event. If that element is no longer in the DOM, fall back to `this.template.querySelector('.graph-canvas-wrapper')` which has `tabindex="0"`. (b) click anywhere outside the menu closes it; (c) selecting a menu item executes the action and closes the menu. **Right-click does NOT change the selected node or the Node Details Panel** - the context menu operates independently of selection. To select a node and open the panel, use left-click. **"Copy API Name" confirmation:** on clipboard success, show a transient toast "Copied API Name: [value]" (auto-dismisses after 3 seconds). On clipboard failure, show error toast: "Could not copy to clipboard. Select and copy the name manually instead." **Esc while menu open:** closes the menu only - does not affect any active focus mode. To clear focus, use the explicit "Clear Focus" button in the toolbar.
...
| Job failed during result serialization | Distinct from "failed mid-way": `ScanResultFileQueueable` failed after all traversal completed. Detected by: `Status__c = 'Failed'` AND `Components_Analyzed__c > 0` AND `Result_File_Id__c` is null AND `Has_Attempted_Result_Save__c = true`. (`Has_Attempted_Result_Save__c` is set to `true` as the first operation in `ScanResultFileQueueable.execute()`, before any DML - this is the distinguishing signal. Without it, mid-traversal failures share the same detection condition and would incorrectly show the "Download Partial Results" button for incomplete data.) Error banner: "Scan analysis is complete but results could not be saved. Your data is available for export for [Retention_Hours__c] hours before it is automatically deleted." Show a conditional "Download Partial Results" button (CSV and JSON) visible only in this specific state. Partial exports use filename: `MetaMapper_[Target_API_Name]_PARTIAL_[YYYYMMDD]_[HHmm].csv` and `.json`. **Zero-node partial download:** If `getNodeHierarchy()` for a `Status__c = Failed` job with `Has_Attempted_Result_Save__c = true` returns an empty node list, do NOT render the "Download Partial Results" button. Instead render: "Scan analysis completed but the results could not be recovered. The dependency data has been deleted. Start a new scan." The download button is only shown when at least one node is available. |
...
|---|---|---|
| Paused, awaiting user action | Both buttons enabled | "Analysis of [Target_API_Name__c] paused - encountered a complex component. You can resume at a slower speed or with current settings." |
...
| `resumeJob()` returned exception | Both buttons re-enabled immediately; toast error shown | "Analysis of [Target_API_Name__c] paused - encountered a complex component. You can resume at a slower speed or with current settings." |
| `resumeJob()` succeeded | Banner replaced by progress view; focus moves to progress bar | Progress status label |
| `resumeJob()` succeeded but `Status__c` does not leave `Paused` within 30 seconds | Both buttons re-enabled; persistent info banner: "Resume is taking longer than expected. The scan will continue when the current step finishes. Try resuming again if this persists." | "Analysis of [Target_API_Name__c] paused - encountered a complex component. You can resume at a slower speed or with current settings." |
...
| "Ask Copilot" button | Fires an `askcopilot` custom event carrying `{ detail: { summaryText: Scan_Summary_Text__c } }` for org-specific Copilot integration. The base `metaMapperApp` implementation is an intentional no-op extension point - org-specific wrapper components may handle this event to open Einstein Copilot or any other AI surface. Conditionally rendered: shown only if Copilot is enabled in the org. **Detection:** call `DependencyJobController.isCopilotEnabled()` (new `@AuraEnabled(cacheable=true)` method, implementation: `return FeatureManagement.checkPermission('Einstein_Copilot');`) on component mount; cache the result for the component lifetime so it is fetched once. If false, render helper text: "Einstein Copilot not available in this org." in place of the button (do not hide silently). **On exception from `isCopilotEnabled()`:** suppress the button silently - do not render the helper text, do not surface the error to the user. Log to browser console only. |
...
| `Is_Platform_Events_Disabled__c` | "Disable live progress updates" | "Turn on if your org is hitting real-time event limits. Progress will refresh every few seconds instead." |
...
| `Max_Concurrent_Jobs__c` | "Max concurrent scans" | "How many MetaMapper scans can run at the same time. Default 2. Raise only for orgs with large async capacity. Set to 0 to disable all new scans org-wide without a deployment." |
...
| `Has_Admin_Customized__c` | "Admin Customized" | Not surfaced in Settings UI - set automatically by the save action. |
...
// +1 per Flow node in current batch if Is_Active_Flows_Only__c = true (1 validation callout per Flow node)
...
// For pathological components, the depth-5 guard in the 414 handler (mark nodes Has_Fetched_Dependencies__c = true
...
// needsFlowValidation = Is_Active_Flows_Only__c AND current batch contains Flow nodes (count, not boolean - 1 callout per node)
...
if (calloutsRemaining < headroom
    ...
    System.enqueueJob(new DependencyQueueable(jobId, isActiveFlowsOnly, overrideBatchSize)); // preserve active batch size override across guardrail chains
    return;
}
```
...
| package.xml | Valid Salesforce deployment manifest grouped by `<types>`. Excludes managed package components. **Namespace detection rule (Round 80):** a candidate namespace prefix is the leading run of letters/digits immediately followed by `__` (`^[A-Za-z][A-Za-z0-9]*(?=__)`). The minimum accepted prefix length depends on how many `__` delimiters the full name contains: **1 character** when the name has exactly one `__` (no trailing custom-suffix segment, e.g. `a__MyClass`); **3+ characters** when the name has two or more `__` delimiters (a trailing `__c`/`__mdt`-style suffix follows the candidate prefix, e.g. `myns__My_Field__c`). This length-dependent minimum is required to satisfy the test matrix below: a short leading word (`My`) immediately followed by another `__`-delimited segment is not treated as a namespace, while a genuinely short registered namespace (`a`) still is, when no trailing suffix follows. For `CustomField` (API name format `Object.Field` or `Object__c.Field__c`), apply the check to the field portion only - the segment after the last dot. **Test cases for namespace detection (enforced in `metaMapperNodeServices.test.js`):** `myns__MyClass` → excluded (namespace `myns`, one `__` delimiter); `My_Custom_Field__c` → included (no `__`-prefixed candidate; `__c` is Salesforce convention not a namespace separator); `myns__My_Field__c` → excluded (namespace `myns`, two `__` delimiters, prefix length 4 ≥ 3); `My__Test__c` → included (two `__` delimiters, prefix `My` length 2 < 3); `_myns__Test` → included (leading underscore is not a valid namespace character); `a__MyClass` → excluded (single-character namespace `a`, one `__` delimiter, length 1 ≥ 1). |
...
| `Is_Platform_Events_Disabled__c` | Checkbox | false | When true, suppresses `Dependency_Scan_Status__e` publish and falls back to polling via `getJobStatus()`. Use when org is approaching the daily Platform Event delivery limit. |
...
| `Max_Concurrent_Jobs__c` | Number | 2 | Maximum number of simultaneously active MetaMapper Queueables. `createJob()` rejects new submissions above this threshold with a user-facing message. Raise for orgs with large flex queue allocations. Set to 0 to disable all new scans org-wide without a deployment (`createJob()` throws a dedicated "scans are currently disabled by your admin" message rather than the generic concurrency-rejection message). |
...
| `Has_Admin_Customized__c` | Checkbox | false | Set to true when an admin explicitly saves the Settings UI. Used by `createJob()` to decide whether to apply sandbox/production default profiles. When false (first-install state), profile overrides apply at runtime without mutating the CMDT record. |
...
1. **Checks `Status__c NOT IN ('Completed', 'Cancelled', 'Failed')` before updating** - `Paused` is intentionally NOT in this list: a Paused job that throws during a resume attempt must transition to Failed with full error detail, not remain stuck in Paused permanently. This prevents a losing race from overwriting a Completed or Cancelled status with Failed.
   3a. Uses `FOR UPDATE` when querying the job record to prevent concurrent failure handler races - via `MetadataScanJobSelector.getForFailedUpdateLocked()`.
...
| `Is_Active_Flows_Only__c` | When checked, inactive Flow versions are excluded from scan results. Reduces scan scope and processing time. Default: true. Unchecking this includes deprecated Flow versions and may significantly increase scan duration. |
...
| `Has_Attempted_Result_Save__c` | Set to true at the start of `ScanResultFileQueueable.execute()`, committed before `Database.setSavepoint()` so a rollback cannot revert this flag. Allows the app to distinguish a serializer failure (traversal completed but saving failed) from a mid-traversal engine failure. When true and Result_File_Id__c is null and Status is Failed, the app surfaces a "Download Partial Results" option. Do not edit manually. |
| `Job_Type__c` | Distinguishes the type of scan job. Dependency_Map = structural metadata dependency traversal (current engine). Text_Search = reserved inactive value for a future content-based search feature. Set to Dependency_Map by createJob() on every new job. Do not create jobs with Text_Search - the UI does not support it. Do not edit manually. |
| `Pause_Reason__c` | Machine-readable reason the scan was paused. Allows the LWC to display the correct banner without parsing Scan_Diagnostic_Log__c. ComponentLimitReached = scan hit Max_Components__c limit. StallDetected = engine detected repeated cycles with no new nodes found. Null when Status is not Paused. Set by the engine only when transitioning Status to Paused. Do not edit manually. |
| `Is_Platform_Events_Auto_Suppressed__c` | Set to true by the engine when org daily Platform Event delivery exceeds 80%. Allows getJobStatus() to return isPeSuppressionActive: true immediately, before the async CMDT Is_Platform_Events_Disabled__c write commits. Cleared on subsequent job starts. Do not edit manually. |
...
| `Ancestor_Id_Index__c` | Internal engine field storing a pipe-delimited set of 6-character tails (last 6 chars via `.right(6)`) of each ancestor `Metadata_Id__c` value. Uses the auto-number suffix portion of the 18-char ID, which is unique per component. Used as a fast pre-screen index: before performing the full delimiter-safe `Ancestor_Path__c` containment check, the engine checks whether the new node's 6-char tail appears in this index. If no match, skip the expensive full-string check. If a match is found, confirm conclusively against `Ancestor_Path__c`. Long Text 32768 required (7 chars per entry; 1,500-depth tree = ~10,500 chars). Do NOT re-query for multi-generation propagation - pass the in-memory map forward. Do not edit manually. |

### Apex Classes

| Class | Description |
|---|---|
| `DependencyJobController` | `@AuraEnabled` controller for all LWC interactions with scan jobs. Invoked by `metaMapperApp`, `metaMapperSearch`, and `metaMapperProgress`. Enforces FLS and CRUD via USER_MODE. Delegates all business logic to services - no SOQL or DML directly. Must NOT be called from async contexts (Batch, Future, Queueable). Exposes: `createJob()`, `getJobStatus()` (response wrapper includes `batchSizeInUse: Integer` - equal to `Batch_Size_Override__c` if non-null, else the CMDT `Scan_Batch_Size__c` value - and `isPeSuppressionActive: Boolean`), `getNodeHierarchy()`, `cancelJob()`, `resumeJob()`, `getObjectList()`, `getComponentCount()`, `isCopilotEnabled()`, `getActiveJobId()` (returns Id of the first Initializing or Processing job for the current user; used by concurrency rejection flow), `getOrgId()` (returns `UserInfo.getOrganizationId()`; used by `metaMapperApp` to build "Open in Setup" deep-link URLs). |
...
| `metaMapperFilters` | Pure utility module with no HTML template and no Apex calls. Manages sessionStorage persistence and schema validation for shared filter state (`metaMapper_filters_v1`). Exports `loadFilters`, `saveFilters`, `validateFilters`, and `DEFAULT_FILTERS`. Imported by `metaMapperResults`. |
| `metaMapperNodeServices` | Pure utility module with no HTML template and no Apex calls. Provides node-level operations shared across Tree and Graph views: `applyFilters`, `buildNodeMap`, `buildTypeCounts`, `extractTypes`, `maxDepth`, `resolveSetupUrl`, `isNamespacePrefixed`. Imported by `metaMapperResults`, `metaMapperGraph`, and `metaMapperExport`. |
| `metaMapperFormatters` | Pure utility module with no HTML template and no Apex calls. Text and time formatting helpers: `formatElapsed`, `truncateAt`, `renderPills`. Imported by `metaMapperProgress`, `metaMapperResults`, `metaMapperGraph`, and `metaMapperComponentDetailsPanel`. |

### Permission Set - MetaMapper_Admin

| Component | Description |
|---|---|
| `MetaMapper_Admin` | Grants full access to MetaMapper. Assign to admins and developers who need to run dependency scans. Provides Create/Read/Update (never Delete) on Metadata_Scan_Job__c and Metadata_Dependency__c, access to the MetaMapper LWC and Apex controller, and Named Credential principal access required for Tooling API callouts. Record deletion is engine-managed only (ring buffer, nightly cleanup) - no user-context delete is granted or needed. Must be assigned before a user can access or operate MetaMapper. Does not grant access to broader org data beyond what the user's existing profile already permits. Also grants the Custom Permission `MetaMapper_Application_Access` which is checked by `ToolingApiHealthCheck` before any callout. This Custom Permission must be explicitly listed in the permission set metadata XML. |
...
| `Is_Platform_Events_Disabled__c` | When enabled, suppresses real-time progress events and the UI falls back to polling every few seconds. Use this if your org is approaching its daily Platform Event delivery limit. Can also be set automatically by the engine if the limit is exceeded. Default: false. |
...
| `Max_Concurrent_Jobs__c` | Maximum number of MetaMapper scans that can run simultaneously in this org. New scan requests are rejected when this limit is reached. Raise this value only for orgs with large async processing capacity. Default: 2. Set to 0 to disable all new scans org-wide without a deployment. |
...
| `Has_Admin_Customized__c` | Set to true when an admin explicitly saves MetaMapper Settings. While false (first-install default), createJob() applies conservative sandbox or production default profiles at runtime without mutating the CMDT record. Once true, the admin's explicit field values take precedence over all runtime profile overrides. Do not edit manually. |
...
- `Ancestor_Id_Index__c` tail shortcut has a negligible probability of false-positive cycle detection (6-char tail collision); the full `Ancestor_Path__c` delimiter-safe containment check is always used to confirm before setting `Is_Circular__c = true`.
