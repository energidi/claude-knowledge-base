...
- `ContentVersion` / `ContentDocumentLink` records are created in SYSTEM_MODE by `ResultSerializerQueueable`. `ContentVersion.FirstPublishLocationId` is set to the job record ID (not a Chatter library) so Salesforce automatically creates the only `ContentDocumentLink` - tied to the job. No additional links to files libraries or other records are created. `ContentDocumentLink.ShareType = 'V'` (viewer) and `Visibility = 'AllUsers'` - "AllUsers" here means "anyone with access to the linked record", which is governed by the job record's Private OWD. Net effect: the file is visible only to the job owner and users with the MetaMapper_Admin permission set. Community/guest users cannot access it.
...
**Detection logic:**
- On each self-chain, increment `Engine_Execution_Count__c` on the Job and compare `Components_Analyzed__c` to the previous value stored in a transient variable.
- If `Engine_Execution_Count__c` increases by `Stall_Detection_Threshold__c` (default 5) without any change in `Components_Analyzed__c`, transition Job to `Status__c = 'Paused'` (new status value), set `Error_Status_Message__c` with diagnostic context, and publish a `Dependency_Scan_Status__e` warning event.
...
| `Max_Nodes__c` | Number | 5000 | Hard cap on `Components_Analyzed__c` per job. When reached, the engine pauses the job and surfaces a warning. Default 5,000 (safe for Developer Sandbox data storage during active scan). Production orgs can raise to 50,000+. Set to 0 to disable the cap. |
...
| `Max_Nodes__c` | 5,000 | 50,000 | Limits peak transient Data Storage during active scan to ~25MB (well within 200MB sandbox limit with headroom). |
...
| `Engine_Execution_Count__c` | Number of times the async engine has processed a batch for this scan. Used to detect stall loops: if this increases by the Stall Detection Threshold without any increase in Components Analyzed, the scan is automatically paused and the user is alerted. Do not edit manually. |
...
| `Cycle_Detection_Index__c` | Internal engine field. Stores an index used to quickly screen for potential ancestry cycles before performing the full string-based cycle check. Allows the engine to skip the expensive full-string scan on the vast majority of nodes, reserving it only for genuine candidates. Only consulted by the cycle detection engine. Do not edit manually. |
