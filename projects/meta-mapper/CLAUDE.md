...
| `Platform_Events_Auto_Suppressed__c` | Checkbox | Set to `true` by `DependencyNotificationService` when auto-suppression fires (org daily PE delivery limit >80% consumed). Read by `getJobStatus()` alongside the CMDT `Disable_Platform_Events__c` flag to derive `peSuppressionActive` in the `JobStatusResult` wrapper - the LWC switches to polling mode immediately on receiving `peSuppressionActive: true`. The CMDT flag is the admin-controlled switch; this field is the runtime auto-suppression signal. Do not edit manually. |
...
| `Admin_Customized__c` | "Admin Customized" | Not surfaced in Settings UI - set automatically by the save action. |
...
| `Admin_Customized__c` | Checkbox | false | Set to true when an admin explicitly saves the Settings UI. Used by `createJob()` to decide whether to apply sandbox/production default profiles. When false (first-install state), profile overrides apply at runtime without mutating the CMDT record. |
...
| `Platform_Events_Auto_Suppressed__c` | Set to true by the engine when org daily Platform Event delivery exceeds 80%. Allows getJobStatus() to return peSuppressionActive: true immediately, before the async CMDT Disable_Platform_Events__c write commits. Cleared on subsequent job starts. Do not edit manually. |
