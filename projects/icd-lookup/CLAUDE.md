...
**Deprecated property - `flowApiName`:** `icdLookup.js` still declares `@api flowApiName;` (not exposed in `icdLookup.js-meta.xml`, so it no longer appears in Flow Builder's property panel). It originally backed a CMDT-driven per-Flow config feature (`ICD_Lookup__mdt`, Apex `getIcdLookupConfig`) that was built, then fully rolled back and removed from the codebase - `ICD_Lookup__mdt` and `getIcdLookupConfig` no longer exist anywhere in `force-app` (last references removed by commit `68802af`, which also deleted the 3 Jest tests that covered them). The `@api` field itself is kept only because Flow version 7 of "Community - Reproductive eTRF Page 4 - Screen Flow" still references it - Salesforce blocks removing an `@api` property that any saved Flow version references. Status: deprecated, kept as unused dead code until further notice (per user direction, July 21, 2026). Removal path, if revisited: update that Flow version to stop passing a value for `flowApiName`, republish it, then delete the `@api flowApiName;` line from `icdLookup.js`.
