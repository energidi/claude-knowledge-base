...
**CMT config change propagation delay:** `getIcdLookupConfig` is `@AuraEnabled(cacheable=true)`, so a user's browser may reuse a previously cached `ICD_Lookup__mdt` record instead of calling the server again. If an admin edits or deactivates a config record (placeholder, help text, required flag, etc.) while a user already has the Flow page open, that user's session may keep showing the old config until they refresh the page or start a new session. There is no in-app indicator of this delay - admins should expect a live config change to require a refreshed session to take effect everywhere.
...
**`notifyOnError(String context, Exception e)`** - private, called on a non-200 `searchIcd10` response after retry, on unexpected `searchIcd10` exceptions (e.g. malformed JSON), and on `getIcdLookupConfig` SOQL failure. **Round 14 fix:** previously the non-200-after-retry branch threw `AuraHandledException` directly without calling `notifyOnError()` first, so a real NIH API outage never alerted the IS team - only unexpected parsing/programming exceptions did. Both failure paths now call `notifyOnError()`.
