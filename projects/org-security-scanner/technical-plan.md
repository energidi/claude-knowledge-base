...
| Status | `Status__c` | Picklist: Pending / Running / Completed / Failed | Drives UI state machine and polling termination |
...
| Total Findings | `TotalFindings__c` | Number(3,0) | Aggregate count across all severities |
| Critical Count | `CriticalCount__c` | Number(3,0) | Drives score ring color and header badge |
| High Count | `HighCount__c` | Number(3,0) | |
| Medium Count | `MediumCount__c` | Number(3,0) | |
| Low Count | `LowCount__c` | Number(3,0) | |
| Info Count | `InfoCount__c` | Number(3,0) | |
...
| Is Partial Scan | `IsPartialScan__c` | Checkbox | True if running user is not a System Administrator - Tooling API results may be incomplete |
| Error Details | `ErrorDetails__c` | LongTextArea(32768) | Full Apex exception text from failed runners for debugging |
| Org Id | `OrgId__c` | Text(18) | `UserInfo.getOrganizationId()` - identifies which sandbox was scanned |
...
| Affected Component Type | `AffectedComponentType__c` | Picklist: Profile / User / ApexClass / LWC / ConnectedApp / Flow / OrgSetting / CustomObject / Field / PermissionSet / StaticResource / Certificate / RemoteSite | Enables Setup navigation and icon selection |
...
| Raw Evidence | `RawEvidence__c` | LongTextArea(32768) | Scrubbed, truncated evidence dump. Processed by `SecScanEvidenceUtil`: max 10,000 chars, regex-scrubbed for password/token/key patterns. Never stores raw Apex source. |
...
**v1 uses KEYSET pagination.** OFFSET-based pagination is NOT used - Salesforce caps SOQL OFFSET at 2,000 rows, making it unsuitable for any findings list of meaningful size.

KEYSET pattern: `WHERE ScanRun__c = :runId AND Id > :lastSeenId ORDER BY Id LIMIT :pageSize`
- First page: `lastSeenId = null` (no Id filter applied)
- Subsequent pages: pass the Id of the last record returned by the previous call
- Constant-time at any depth - no row-skipping overhead
- Controller signature: `getCurrentScanFindings(String scanRunId, String lastSeenId, Integer pageSize)`
- LWC tracks `lastSeenId` (the Id of the last row in the current page) instead of an offset counter
- `hasMore` returned when result size equals `pageSize`

The data retention LIMIT/OFFSET query in `SecScanOrchestrator` is exempt - it runs against `SecurityScanRun__c` (max 5 records) and uses OFFSET on a tiny dataset, well within the 2,000 cap.
...
|---|---|
| `Status__c` | Pending: scan is queued. Running: scan is in progress. Completed: all categories finished. Failed: scan could not complete. |
...
| `OrgId__c` | Salesforce Organization ID of the sandbox that was scanned. Used to identify which environment this run belongs to when reviewing historical data. |
...
| `RawEvidence__c` | Automatically scrubbed technical evidence captured at scan time. Passwords, tokens, and secret-pattern strings are masked with asterisks. For regex-based findings, captures 5,000 characters surrounding the match so the actual violation is visible. Hard cap of 10,000 characters. Never contains raw Apex source code. |
...
SecScanEvidenceUtil.cls           - Context-aware evidence extraction before writing to RawEvidence__c:
                                    ...
                                    2. Match-centered extraction: capture 5,000 chars before and
                                       after the regex match that triggered the finding, ensuring
                                       the actual violation is preserved in the output.
...
SecScanOrchestrator.cls           - Entry point Queueable
                                    Runs data retention cleanup first (keep 5 most recent runs)
...
- Before each page: check `Limits.getHeapSize() / Limits.getLimitHeapSize()`
- If heap > 70%: save findings so far, enqueue continuation Queueable for same category index with `pageOffset`, then stop current job
...
Developer sandboxes: 200MB storage limit. `SecScanOrchestrator` runs cleanup before creating a new run:

```apex
// Read retention limit from Custom Metadata (Default record deployed with package)
...
// Keep only the N most recent completed runs
List<SecurityScanRun__c> oldRuns = [
    SELECT Id FROM SecurityScanRun__c
    WHERE Status__c = 'Completed'
    ORDER BY CompletedAt__c DESC
    LIMIT 10000 OFFSET :maxRuns
];
if (!oldRuns.isEmpty()) delete oldRuns; // cascade deletes all child findings

// Also clean up orphaned stuck runs older than 24 hours
delete [SELECT Id FROM SecurityScanRun__c
        WHERE Status__c = 'Running'
        AND StartedAt__c < :DateTime.now().addHours(-24)];
```
...
- All DML writes: `Database.insert(records, AccessLevel.USER_MODE)`
- Where `WITH USER_MODE` would strip needed fields: `Security.stripInaccessible(AccessType.READABLE, records)`
...
| Partial scan | Warning banner: "Running as non-System Administrator - results may be incomplete." |
...
- Click button: `lightning-modal` confirmation before scan starts
...
Score calculation runs in LWC from the live findings array - no stored score field on `SecurityScanRun__c`. Recalculates automatically when a finding status changes to Remediated or False Positive.
...
**Decision: Slide-in right panel** (not modal) - keeps list visible for sequential browsing. 45% width. Full-screen on mobile.
...
- `AcknowledgedBy__c` and `AcknowledgedDate__c` set on every save where status transitions away from Open. Overwritten on each subsequent change (e.g. Open -> Acknowledged -> Risk Accepted updates both fields each time). Tracks the last admin who acted, not the first. This is intentional - audit trail is available via Salesforce field history tracking, not these fields.
...
- Shows "Storing X of 5 max runs. Oldest auto-deleted."
...
- Below 768px: detail panel = full-screen overlay
...
export const SCAN_STATUS   = { PENDING: 'Pending', RUNNING: 'Running', COMPLETED: 'Completed', FAILED: 'Failed' };
...
**`@wire`:** `getScanRuns`, `getCurrentScanFindings` (reactive to `currentScanId`), `getFindingDetail` (reactive to `selectedFindingId`), `getOrgSecuritySettings` (score weights + retention limit for score ring and history tab)

**Imperative:** `runSecurityScan`, `getScanRunStatus` (polled), `updateFindingStatus`, `bulkUpdateFindingStatus`, `cancelScan`, `exportFindingsCsv`

### Apex Controller Methods

**`SecScanController`:**
- `runSecurityScan()` - validates PS, validates `Organization.IsSandbox = true`, checks sysadmin flag, captures `sessionId`, enqueues orchestrator. Returns `SecScanApiResponse<Id>`.
- `getScanRuns()` - list of `SecurityScanRun__c` with counts. `SecScanApiResponse<List>`.
- `getOrgSecuritySettings()` - returns `OrgSecurityScanner_Setting__mdt.getInstance('Default')` field values as a plain map. Used by score ring and history tab. Cacheable=true.
- `getScanRunStatus(String scanRunId)` - `{ status, failedCategories, isPartialScan }`. `SecScanApiResponse`.
- `cancelScan(String scanRunId)` - marks cancelled.
- `exportFindingsCsv(String scanRunId)` - generates CSV, stores as `ContentVersion` linked to run, returns `ContentDocumentId`. Never returns raw String.

**`SecScanFindingsController`:**
- `getCurrentScanFindings(String scanRunId, String lastSeenId, Integer pageSize)` - KEYSET paginated (default 100). `lastSeenId` = null for first page, Id of last row for subsequent pages. Returns `SecScanApiResponse<FindingsPageDTO>` where `FindingsPageDTO` has `List<SecurityFinding__c> findings`, `Boolean hasMore`, `String lastSeenId`.
- `getFindingDetail(String findingId)` - single finding all fields.
- `updateFindingStatus(String findingId, String status, String note)` - returns updated record.
- `bulkUpdateFindingStatus(List<String> findingIds, String status, String note)` - returns count.
...
| 2 - Apex Foundation | `SecScanException`, `SecScanApiResponse`, `SecScanEvidenceUtil`, `SecScanFindingDTO`, `SecScanToolingService`, `SecScanMetadataService`, `SecScanCategoryRunner` | All runners extend/use these |
| 3 - Apex Runners | All 13 `SecScanRunner*.cls` | Extend base class from Phase 2 |
| 4 - Apex Orchestration | `SecScanRunnerChain`, `SecScanOrchestrator` | Reference runners and each other |
...
**CLAUDE.md contents:** App name, tech stack (SFDX v66.0, Apex + LWC + SLDS), sandbox-only restriction, no external callouts, class prefix `SecScan`, LWC prefix `security`, deployment order constraint, 76 checks across 13 categories.
...
| Paginated fetch (KEYSET) | `getCurrentScanFindings(runId, null, 10)` returns 10 rows and `hasMore=true`; calling with `lastSeenId` = last returned Id returns next 10 rows; final page returns `hasMore=false` |

### Negative Tests
| Step | Expected |
|---|---|
| Run scan outside sandbox | `SecScanException` thrown, no `SecurityScanRun__c` created |
